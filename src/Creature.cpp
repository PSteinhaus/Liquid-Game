...
void Tree::growRoot() {
	...
	for( engine.rootIterationBegin(&xPos,&yPos); xPos!=engine.rootIterationEnd(); engine.rootIterationNext(&xPos,&yPos) ) {
		...
		while( currentDirection != NEUTRAL && !(fieldAdded && fieldAddedOuter) ) {							// go through all directions
			if( !engine.checkRootDirection( xPos, yPos, currentDirection) ) {	
				if( engine.getHeight(xPos,yPos)>=fluidNeededForGrowth && !fieldAdded
					&& engine.rootAllowed(xPos,yPos,currentDirection))			// if this field is missing an allowed root add it to the list
				{
				...
				if( engine.getHeight(xPos+dx,yPos+dy)>=fluidNeededForGrowth && !engine.checkRootDirection( xPos+dx, yPos+dy, opposingDirection )
	...
	if( rand() % (edgeCount+outerEdgeCount) < edgeCount ) {		// decide randomly whether to pick an edge-field or an outer-edge-field
		...
		while( true ) {
			...
			if( engine.getHeight(chosenX,chosenY)>=fluidNeededForGrowth && !engine.checkRootDirection( chosenX, chosenY, randomDirection )
		...
		while( true ) {
			...
			if( engine.getHeight(chosenX+dx,chosenY+dy)>=fluidNeededForGrowth && engine.checkRootDirection( chosenX, chosenY, randomDirection )
				&& !engine.checkRootDirection( chosenX+dx, chosenY+dy, opposingDirection ) && engine.rootAllowed(chosenX+dx, chosenY+dy, opposingDirection) )
			{  
